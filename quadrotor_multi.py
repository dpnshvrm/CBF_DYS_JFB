...
# ── Physical parameters ────────────────────────────────────────────────────────
m = 0.5    # mass [kg]
g = 1.0    # normalised gravity (matches reference / LaTeX experiments)

T_hover = m * g   # hover thrust per agent = 0.5

# ── State / control layout ─────────────────────────────────────────────────────
# Per-agent state  (12-D):  [x, y, z, ψ, θ, φ, vx, vy, vz, vψ, vθ, vφ]
# Per-agent control (4-D):  [u, τψ, τθ, τφ]
#
# Dynamics (from LaTeX / reference code):
#   ẋ  = vx,  ẏ  = vy,  ż  = vz
#   ψ̇  = vψ,  θ̇  = vθ,  φ̇  = vφ          ← simplified kinematics (no W matrix)
#   v̇x = (u/m) f7(ψ,θ,φ)
#   v̇y = (u/m) f8(ψ,θ,φ)
#   v̇z = (u/m) f9(θ,φ) − g
#   v̇ψ = τψ,  v̇θ = τθ,  v̇φ = τφ          ← direct angular acceleration
#
# where:
#   f7 = sin(ψ)sin(φ) + cos(ψ)sin(θ)cos(φ)
#   f8 = −cos(ψ)sin(φ) + sin(ψ)sin(θ)cos(φ)
#   f9 = cos(θ)cos(φ)
...
def thrust_direction(psi, theta, phi):
    """Unit thrust vector in world frame  [f7, f8, f9].
...
# ── Dynamics ───────────────────────────────────────────────────────────────────
def f(z, u, t=None):
    """Quadrotor dynamics  ż = f(z, u)  matching the LaTeX model.
...
def G(z, p_target):
    ...
    p_target : (N, 3) — per-agent 3-D target position.
    """
    B = z.shape[0]
    v = z.reshape(B, n_agent, 12)
    pos     = v[:, :, 0:3]
...
# ── HOCBF  (relative degree 2 w.r.t. thrust u) ────────────────────────────────
def gamma(x):
    return 1.0 * x


def barrier_function(z, center, r_obs, eps):
    """h = ‖pos − center‖² − r² − ε²  →  (B, N)."""
...
def construct_cbf_constraints(z, centers, r_obs, eps):
    """Build per-sample linear CBF constraint  K u ≤ d  (enforces ψ₂ ≥ 0).

    Only the thrust u_i appears in the second Lie derivative of h;
    angular accelerations τ do not.

    For agent i, obstacle j:
        K_row[4i]        = −(2/m)(dp_i · td_i)   ← thrust column
        K_row[4i+1:4i+4] = 0                       ← τ columns absent
        d_row            = 2‖vel_i‖² − 2g·dp_iz + γ(Lf_h_i) + γ(ψ₁_i)
...
# ── Initial conditions ─────────────────────────────────────────────────────────
def sample_initial_condition(batch_size=64, z0_std=0.05):
    """Agents equally spaced on a circle at z = 1, hovering (zero vel / rates)."""
    center    = torch.tensor([1.5, -0.5], device=device)
    radius    = 0.3                             # tight cluster below obstacle field
    ...
    z0_view[:, :, 0] = center[0] + radius * torch.cos(angles_xy)   # x
    z0_view[:, :, 1] = center[1] + radius * torch.sin(angles_xy)   # y
    z0_view[:, :, 2] = 1.0                                          # z
    ...
    z0_view[:, :, 2]  += 0.1 * z0_std * torch.randn(batch_size, n_agent, device=device)
...
def compute_loss(u, z0, nt, f, p_target, obstacle_centers, r_obstacle, eps_safe,
                 ...
                 verbose=False, return_traj=True, dt=None):
    """Roll out the closed-loop trajectory under the CBF-projected policy.

    Network output is treated as a deviation from hover:
        u_i  = m·g  + net_output[u_i]
        τ_i  =  0   + net_output[τ_i]
    """
    running_cost = 0.0
    z  = z0
...
# ── Plotting ───────────────────────────────────────────────────────────────────
def plot_trajectory(traj, obstacle_centers, obstacle_radius, p_target,
                    title="Quadrotor Trajectories", eps_safe=0.1, n_traj=10):
    """3-D trajectory plot.

    traj             : numpy (B, STATE_DIM, T+1)
    obstacle_centers : list of numpy (1, 3)
    p_target         : tensor (N, 3)
    """
